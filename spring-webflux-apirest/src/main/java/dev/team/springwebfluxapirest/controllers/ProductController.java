package dev.team.springwebfluxapirest.controllers;
...
public class ProductController {
    ...
    @PostMapping("/v2")
    public Mono<ResponseEntity<Product>> crearConFoto(Product producto, @RequestPart FilePart file) {
        ...
        return file.transferTo(new File(path + producto.getPhoto())).then(service.save(producto))
    ...
    @PostMapping
    public Mono<ResponseEntity<Map<String, Object>>> crear(@Valid @RequestBody Mono<Product> monoProduct) {

        Map<String, Object> respuesta = new HashMap<String, Object>();

        return monoProduct.flatMap(producto -> {
            if (producto.getCreatedAt() == null) {
                producto.setCreatedAt(new Date());
            }

            return service.save(producto).map(p -> {
                respuesta.put("product", p);
